...
public class InitializeHandlerTest {
    ...
    public void setUp() throws JsonProcessingException {
        ...
        undertow = Undertow.builder()
                ...
                .setHandler(new RoutingHandler()
                                .add("GET", "/v1/admin/initialize", new InitializeHandler(db))
                )
                .build();
        undertow.start();
    ...
    public void shouldInitialize() throws IOException {
        ...
        assertEquals("Initialized!", actual);

        try (Transaction tx = db.beginTx() ) {
            assertEquals("identity.identity IS UNIQUE", db.schema().getConstraints(Labels.Identity).iterator().next().toString());
            assertEquals("page.url IS UNIQUE", db.schema().getConstraints(Labels.Page).iterator().next().toString());
        }
    }
}